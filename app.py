...
.cam-placeholder {
  ...
  background:#0b0b17; border-radius:12px; border:1px solid #13132a;
  color:#2d2d45 !important; font-size:0.9rem; text-align:center; padding:1rem;
}
...
# flush proactive messages from background thread into chat
...
with c1:
    typed_msg = st.text_input(
        "msg",
        placeholder="Type a question or just press 🎙 to speak…",
        key="typed_input",
...
if st.session_state.processing:
    ...
    last_user = next(
        (m["text"] for m in reversed(st.session_state.messages)
         if m["role"] == "user"),
        None,
    )
    if last_user:
...
with cam_col:
    ...
    if camera_image is not None:
        ...
        if frame is not None:
            ...
            display = (
                engine.annotated
                if engine.annotated is not None and engine.query
                else frame
            )
        ...
        st.markdown(
            ...
            '📷 Allow camera access and click Take Photo</div>',
            unsafe_allow_html=True,
        )
...
# ── chat panel ─────────────────────────────────────────────────────────────────
with chat_col:
    st.markdown('<div class="chat-heading">Conversation</div>',
                unsafe_allow_html=True)

    if not st.session_state.messages:
        st.markdown(
            '<div class="bubble system">'
            '👋 Hey! Ask me anything — I can see what\'s around you.</div>',
            unsafe_allow_html=True,
        )
    else:
        ...
        for m in st.session_state.messages[-20:]:
            html += (
                f'<div class="bubble {m["role"]}">{m["text"]}'
